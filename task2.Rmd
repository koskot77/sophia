...
Let us check how often patterns of specific lengths repeat in the data. From now on for the presentation purposes I use unique *pattern ID* numbers in place of the lengthy symbolic sequences. You can associate the *pattern ID* to a sequence using my more interactive [**shinyapp tool**](https://koskot77.shinyapps.io/sophia/).
...
Interesting features on these plots are horizontal and vertical clusters of dots. Although many of these dots correspond to a rather similar sequences, there still seem to be several different types of patterns there. Perhaps, those patterns mostly show up correlated in the sequences.
...
p10 <- ggplot(data=an10[an10$count>30000,],aes(x=meanQual,y=count)) + geom_point(size=1) +
       labs(x=expression(paste("Average quality (",Sigma,q[i]," / length)")),y="# of patterns",title="Pattern length = 10")

p20 <- ggplot(data=an20[an20$count>5000,],aes(x=meanQual,y=count)) + geom_point(size=1) +
...
- 432 records have base labeled 'N' with the corresponding quality 0. Patterns that included such terms were dismissed from the study.
